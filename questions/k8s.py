...
def k8sQuestion(directory):
    ...
    ktEtcdClusterHostname = str(
        input("Enter the hostname for your primary K8s node (Default: k8s-primary-1 : ") or "k8s-primary-1"
    )
    while True:
    ...
    ktETCD_INITIAL_CLUSTER = str(
        input(
            "What ETCD Initial Cluster do you want to use (Default: '{}:{}') : ".format(
                ktEtcdClusterHostname, ktEtcdClusterIP
            )
        )
        or "{}:{}".format(ktEtcdClusterHostname, ktEtcdClusterIP)
    )
    ...
    createK8sEnvFile(
        os=ktOS,
        version=ktVERSION,
        container_runtime=ktCONTAINER_RUNTIME,
        cni=ktCNI_PROVIDER,
        etcd_initial_cluster=ktETCD_INITIAL_CLUSTER,
        etcd_ip=ktETCD_IP,
        k8s_advertise=ktKUBE_API_ADVERTISE_ADDRESS,
        dashboard=ktINSTALL_DASHBOARD,
